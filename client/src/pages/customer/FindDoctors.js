                        ...
                        doctors.map((ele) => {
                            return <div key={ele._id} className=" border-b border-gray-500 mt-2 mb-2  flex flex-row ">
                                ...
                                <div className=" p-4 w-72 ">
                                    ...
                                    <p className="mt-1  text-sm font-semibold opactity-80"> Gender : <span className="ml-3"> {ele?.profile?.gender}</span> </p>
                                    <p className=" mt-3 font-semibold "> Doctor Address :  </p>
                                    <p className="text-sm font-semibold  mt-1">Buliding : <span className="ml-2 opacity-80"> {ele?.address?.buildingNo}</span> </p>
                                    <p className="text-sm font-semibold  mt-1">Street : <span className="ml-2 opacity-80"> {ele?.address?.street}</span> </p>
                                    <p className="text-sm font-semibold  mt-1">City : <span className="ml-2 opacity-80"> {ele?.address?.city}</span> </p>
                                    <p className="text-sm font-semibold  mt-1">State : <span className="ml-2 opacity-80"> {ele?.address?.state}</span> </p>
                                </div>
                                <div className="p-4 w-72">
                                    ...
                                    <div className="flex justify-center items-center mt-5">
                                        <button 
                                            ...
                                            onClick={ ( ) => {
                                                navigate("/book-appointment", { state : ele.userId})
                                            }}
                                        > 
                                            Book Appointment
                                        </button>
